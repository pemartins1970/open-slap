...
class DatabaseManager:
    ...
    def _ensure_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                conn.execute("ALTER TABLE user_soul_events ADD COLUMN salience REAL DEFAULT 0.5")
            except Exception: pass
            try:
                conn.execute("ALTER TABLE user_soul_events ADD COLUMN confidence REAL DEFAULT 0.8")
            except Exception: pass
            try:
                conn.execute("ALTER TABLE user_soul_events ADD COLUMN last_used_at TIMESTAMP")
            except Exception: pass
            try:
                conn.execute("ALTER TABLE user_soul_events ADD COLUMN pinned INTEGER DEFAULT 0")
            except Exception: pass
            try:
                conn.execute("ALTER TABLE user_soul_events ADD COLUMN decay_rate REAL DEFAULT 0.05")
            except Exception: pass
    ...
    def create_conversation(
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "INSERT INTO conversations (user_id, session_id, title, kind) VALUES (?, ?, ?, ?)",
                (user_id, session_id, _redact_text(title) or "", kind or "conversation"),
            )
    ...
    def get_user_conversations(
        self, user_id: int, kind: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conversations = conn.execute(
                f"""
                SELECT id, session_id, title, created_at, updated_at, kind,
    ...
    def add_task_todo(self, user_id: int, conversation_id: int, text: str) -> int:
        with sqlite3.connect(self.db_path) as conn:
            cur = conn.execute(
                """
                INSERT INTO task_todos (user_id, conversation_id, text)
                VALUES (?, ?, ?)
            """,
                (user_id, conversation_id, _redact_text(text) or ""),
            )
    ...
    def list_task_todos(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            rows = conn.execute(
                f"""
                SELECT id, conversation_id, text, status, created_at, updated_at
                FROM task_todos
                WHERE {where}
    ...
    def get_conversation_by_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conversation = conn.execute(
                """
                SELECT id, user_id, session_id, title, kind, created_at, updated_at
                FROM conversations
                WHERE session_id = ?
                ORDER BY created_at DESC
                LIMIT 1
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                ...
                ON CONFLICT(user_id, connector_key) DO UPDATE SET
                  ...
                  updated_at=CURRENT_TIMESTAMP
                """,
                (int(user_id), str(connector_key or "").strip().lower(), str(secret_ciphertext)),
            )
            conn.commit()
    ...
    def create_telegram_link_code(self, user_id: int, ttl_seconds: int = 600) -> Dict[str, Any]:
    ...
    def upsert_telegram_link(self, user_id: int, telegram_user_id: str, chat_id: str) -> None:
    ...
    def get_telegram_linked_user_id(self, telegram_user_id: str, chat_id: str) -> Optional[int]:
        tuid = str(telegram_user_id or "").strip()
        cid = str(chat_id or "").strip()
        if not (tuid and cid):
            return None
    ...
    def update_user_system_profile_data(
        ...
        self.upsert_user_system_profile(int(user_id), current.get("markdown") or "", merged)
    ...
    def upsert_message_feedback(self, user_id: int, message_id: int, rating: int) -> None:
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
            ...
            for i, task in enumerate(tasks):
                cur = conn.execute(
                    ...
                    (user_id, parent_conversation_id,
                     task.get("title", ""), task.get("skill_id"), i),
                )
    ...
    def record_expert_rating(self, user_id: int, expert_id: str, positive: bool) -> None:
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
    ...
    def get_expert_rating_summary(self, user_id: int) -> dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            return {
                ...
                if r["total_responses"] > 0 else 0.5
                for r in rows
            }
    ...
    def get_message_with_preceding_user_message(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (msg["conversation_id"], message_id)).fetchone()
            if not prev:
                return None
    ...
    def update_orchestration_run(
        self, run_id: int, status: str, log: list
    ) -> None:
        import json
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
...
# Funções auxiliares
def create_conversation(
    user_id: int, session_id: str, title: str, kind: str = "conversation"
) -> int:
    return db_manager.create_conversation(user_id, session_id, title, kind=kind)


def get_user_conversations(
    user_id: int, kind: Optional[str] = None
) -> List[Dict[str, Any]]:
    return db_manager.get_user_conversations(user_id, kind=kind)
...
def add_task_todo(user_id: int, conversation_id: int, text: str) -> int:
...
def update_task_todo(
    ...
    return db_manager.update_task_todo(user_id, todo_id, text=text, status=status)
...
def update_user_system_profile_data(user_id: int, patch: Dict[str, Any]) -> Dict[str, Any]:
...
def update_plan_task_status(task_id: int, status: str, conversation_id: Optional[int] = None) -> None:
