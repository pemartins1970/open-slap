...
const App = () => {
  ...
  const orchLoadingRef = useRef(false);
  const planAutoApprovedRef = useRef({});
  ...
  const [planAutoApproveInformational, setPlanAutoApproveInformational] = useState(() => {
    const stored = localStorage.getItem('planAutoApproveInformational');
  ...
  // PM Expert Mode / TAP States
  const [showTapModal, setShowTapModal] = useState(false);
  const [tapEditingProjectId, setTapEditingProjectId] = useState(null);
  ...
  const [expertTab, setExpertTab] = useState('tap');
  const [showAddStakeholder, setShowAddStakeholder] = useState(false);
  const [stakeholderForm, setStakeholderForm] = useState({ name: '', role: '', impact: 'Medium', channel: 'Daily' });
  const [showAddRisk, setShowAddRisk] = useState(false);
  const [riskForm, setRiskForm] = useState({ description: '', category: 'Technical', probability: 'Medium', impact: 'Medium', mitigation: '' });
  const [showAddRelease, setShowAddRelease] = useState(false);
  const [releaseForm, setReleaseForm] = useState({ name: '', date: '', scope: '', status: 'Pendente' });
  // Notes / Brain
  const [notes, setNotes] = useState([]);
  const [selectedNote, setSelectedNote] = useState(null);
  const [noteSearchQuery, setNoteSearchQuery] = useState('');
  const [notesFilterProjectId, setNotesFilterProjectId] = useState('all');
      ...
      return (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--green)', letterSpacing: '0.04em' }}>
              PLAN
            </div>
            <div style={{
              fontFamily: 'var(--mono)',
              ...
              <div style={{ display: 'grid', gap: '6px' }}>
                {tasks.map((t, i) => (
                  <div key={`${t.title}-${t.skill_id}-${i}`} style={{ display: 'flex', gap: '10px', alignItems: 'baseline' }}>
                    <div style={{ width: '18px', textAlign: 'center', color: 'var(--green)', fontFamily: 'var(--mono)' }}>•</div>
  ...
  // Onboarding check rápido (independente do carregamento pesado)
  useEffect(() => {
    if (!isAuthenticated || onboardingChecked) return;
    setOnboardingChecked(true);
    let hide = false;
    try { hide = localStorage.getItem('open_slap_onboarding_hide_v1') === '1'; } catch {}
    if (hide) return;
  ...
  const handleSaveNote = async (note) => {
    if (!token) return;
    if (!note.title.trim()) {
      alert(lang === 'pt' ? 'Por favor, insira um título para a nota.' : 'Please enter a note title.');
      return;
    }
    setNoteSaving(true);
    try {
      ...
      const body = {
        ...
        tags: note.tags || '',
        project_id: note.project_id ? parseInt(note.project_id) : null
      };
      ...
      if (res.ok) {
        const data = await res.json();
        if (isNew && data.id) {
          setSelectedNote({ ...body, id: data.id });
        } else {
          setSelectedNote({ ...note, ...body });
        }
        fetchNotes();
      } else {
        const errData = await res.json().catch(() => ({}));
        alert(`Failed to save note: ${errData.detail || 'Unknown error'}`);
      }
    } catch (err) {
  ...
  const handleDeleteNote = async (noteId) => {
    ...
    if (!confirm(lang === 'pt' ? 'Tem certeza que deseja excluir esta nota?' : 'Are you sure you want to delete this note?')) return;
    try {
      const headers = getAuthHeaders();
      ...
      if (res.ok) {
        ...
        const errData = await res.json().catch(() => ({}));
        alert(`Failed to delete note: ${errData.detail || 'Unknown error'}`);
      }
    } catch (err) {
  ...
  const handleAddStakeholder = async () => {
    ...
    let list = [];
    try {
      list = JSON.parse(activeProject.stakeholders_json || '[]');
    } catch {
      list = [];
    }
    const updated = [...list, { ...stakeholderForm, id: Date.now() }];
    const headers = getAuthHeaders();
    headers['Content-Type'] = 'application/json';
    const res = await fetch(`/api/projects/${activeProjectId}`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ stakeholders_json: JSON.stringify(updated) })
    });
    if (res.ok) {
      setShowAddStakeholder(false);
      setStakeholderForm({ name: '', role: '', impact: 'Medium', channel: 'Daily' });
      await loadProjects();
    }
  };
  ...
  const handleAddRisk = async () => {
    ...
    let list = [];
    try {
      list = JSON.parse(activeProject.risk_matrix_json || '[]');
    } catch {
      list = [];
    }
    const updated = [...list, { ...riskForm, id: Date.now() }];
    const headers = getAuthHeaders();
    headers['Content-Type'] = 'application/json';
    const res = await fetch(`/api/projects/${activeProjectId}`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ risk_matrix_json: JSON.stringify(updated) })
    });
    if (res.ok) {
      setShowAddRisk(false);
      setRiskForm({ description: '', category: 'Technical', probability: 'Medium', impact: 'Medium', mitigation: '' });
      await loadProjects();
    }
  };
  ...
  const handleAddRelease = async () => {
    ...
    let list = [];
    try {
      list = JSON.parse(activeProject.release_plan_json || '[]');
    } catch {
      list = [];
    }
    const updated = [...list, { ...releaseForm, id: Date.now() }];
    const headers = getAuthHeaders();
    headers['Content-Type'] = 'application/json';
    const res = await fetch(`/api/projects/${activeProjectId}`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ release_plan_json: JSON.stringify(updated) })
    });
    if (res.ok) {
      setShowAddRelease(false);
      setReleaseForm({ name: '', date: '', scope: '', status: 'Pendente' });
      await loadProjects();
    }
  };
  ...
  const startOrchestration = async (convId) => {
    if (orchLoadingRef.current) return;
    orchLoadingRef.current = true;
    try {
      const headers = getAuthHeaders();
      if (!headers.Authorization) return;
      headers['Content-Type'] = 'application/json';
      ...
      if (!res.ok) {
        ...
        setGenericModal({ title: 'Orchestration error', body: msg });
        return;
      }
      ...
      orchPollRef.current = setInterval(async () => {
        try {
          ...
          if (pd.status === 'completed' || pd.status === 'failed') {
            ...
            await loadTasks();
            // Reload conversation messages to show agent results
            if (activePlanConvId) {
              await loadConversation(activePlanConvId);
            }
          }
        } catch {}
      }, 2000);
    } catch {}
    finally { orchLoadingRef.current = false; }
  };
  ...
  // Auto-approve plan blocks based on settings
  React.useEffect(() => {
    if (!currentConversation || orchStatus === 'running') return;
    for (const msg of messages) {
      if (planAutoApprovedRef.current[msg.id]) continue;
      if (msg.role !== 'assistant' || msg.streaming) continue;
      const parsed = parseCommandRequestsFromContent(msg.content || '');
      const blocks = parseMessageBlocks(parsed.text || '');
      const planBlock = blocks?.find(b => b.type === 'plan_auto' || (b.type === 'plan' && planAutoApproveAll));
      if (!planBlock) continue;
      const tasks = parsePlanTasksFromText(planBlock.content || '');
      const tlist = Array.isArray(tasks) ? tasks : [];
      if (!tlist.length) continue;
      planAutoApprovedRef.current[msg.id] = true;
      setShowExecutionPanel(true);
      setActivePlanMessageId(msg.message_id || null);
      setActivePlanLocalMsgId(msg.id || null);
      approvePlan(currentConversation, tlist).then((ok) => {
        if (!ok) return;
        setApprovedPlanLocalMsgIds(prev => ({ ...prev, [msg.id]: true }));
  ...
  }, [messages, currentConversation, planAutoApproveAll]);
  ...
  const createNewConversation = async (opts = {}) => {
    try {
      ...
      if (!headers.Authorization) {
        setGenericModal({
          title: t('error'),
          body: lang === 'pt' ? 'Não autenticado. Faça login novamente.' : 'Not authenticated. Please sign in again.'
        });
        return null;
      }
      headers['Content-Type'] = 'application/json';
      ...
      if (!response.ok) {
        const detail = data?.detail || (lang === 'pt' ? 'Falha ao criar conversa.' : 'Failed to create conversation.');
        setGenericModal({ title: t('error'), body: String(detail) });
        return null;
      }
      if (!data?.conversation_id || !data?.session_id) {
        setGenericModal({
          title: t('error'),
          body: lang === 'pt' ? 'Resposta inválida ao criar conversa.' : 'Invalid response while creating conversation.'
        });
        return null;
      }
      ...
      setGenericModal({
        title: t('error'),
        body: lang === 'pt' ? 'Erro ao criar conversa.' : 'Error creating conversation.'
      });
      return null;
    }
  };
  ...
  function parseAssistantDirectivesFromText(text) {
    ...
    const cleanPart = (raw) => {
      ...
      cleaned = cleaned.replace(/\[\[add_step:\s*([^\]]+)\]\]/gi, (_m, desc) => {
        actions.push({ type: 'add_step', description: String(desc || '').trim() });
        return '';
      });
  ...
  function runUiActions(actions) {
    ...
    for (const a of actions) {
      ...
      if (a?.type === 'add_step' && a?.description) {
        setSabrinaSteps(prev => [...prev, {
  ...
  const buildStartProjectPrompt = () => (
    ...
    'Objetivo: Agir como uma consultora proativa para extrair a visão do usuário e organizar o kickoff do projeto.\n' +
    'Siga as instruções de tom e fraseado dinâmicos fornecidas pelo sistema.'
  );
  ...
  const createNewTask = async (opts = {}) => {
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        const detail = data?.detail || (lang === 'pt' ? 'Falha ao criar task.' : 'Failed to create task.');
        setGenericModal({ title: t('error'), body: String(detail) });
        return null;
      }
      if (!data?.conversation_id || !data?.session_id) {
        setGenericModal({
          title: t('error'),
          body: lang === 'pt' ? 'Resposta inválida ao criar task.' : 'Invalid response while creating task.'
        });
        return null;
      }
      ...
      setGenericModal({
        title: t('error'),
        body: lang === 'pt' ? 'Erro ao criar task.' : 'Error creating task.'
      });
      return null;
    }
  };
          ...
          !isAuthenticated ? <Navigate to="/login" replace /> : (
            <div style={styles.app}>
              ...
              <OnboardingModal
                ...
                setSoulMarkdown={setSoulMarkdown}
                setCenterView={setCenterView}
                startNewConversationWithPrompt={startNewConversationWithPrompt}
                buildStartProjectPrompt={buildStartProjectPrompt}
              ...
              {showTapModal ? (
                <div style={styles.modalOverlay} onClick={() => setShowTapModal(false)}>
                  <div style={{ ...styles.modal, maxWidth: '620px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
                    <div style={styles.modalHeader}>
                      <div style={styles.modalTitle}>
                        📋 {tapEditingProjectId ? (lang === 'pt' ? 'Editar Termo de Abertura (TAP)' : 'Edit Project Charter (TAP)') : (lang === 'pt' ? 'Criar Novo Projeto com TAP' : 'Create New Project with TAP')}
                      </div>
                      <button style={styles.iconButton} onClick={() => setShowTapModal(false)} title={t('close')}>×</button>
                    </div>
                    <div style={{ ...styles.modalBody, overflowY: 'auto', flex: 1, padding: '16px', display: 'grid', gap: '12px' }}>
                      <div style={styles.settingsField}>
                        <div style={styles.settingsLabel}>
                          {lang === 'pt' ? 'Nome do Projeto' : 'Project Name'} <span style={{ color: 'var(--red)' }}>*</span>
                        </div>
                        <input
                          style={styles.settingsInput}
                          disabled={!!tapEditingProjectId}
                          value={tapFormData.name}
                          onChange={e => setTapFormData({ ...tapFormData, name: e.target.value })}
                          placeholder={lang === 'pt' ? 'Ex: Desenvolvimento do Novo App' : 'Ex: New App Development'}
                        />
                      </div>

                      <div style={styles.settingsField}>
                        <div style={styles.settingsLabel}>
                          1. {lang === 'pt' ? 'Justificativa e Objetivo de Negócio' : 'Business Case & Purpose'}
                        </div>
                        <textarea
                          style={{ ...styles.settingsTextarea, minHeight: '60px' }}
                          value={tapFormData.purpose}
                          onChange={e => setTapFormData({ ...tapFormData, purpose: e.target.value })}
                          placeholder={lang === 'pt' ? 'Qual problema o projeto resolve...' : 'What problem does the project solve...'}
                        />
                      </div>

                      <div style={styles.settingsField}>
                        <div style={styles.settingsLabel}>
                          2. {lang === 'pt' ? 'Objetivos Mensuráveis e Métricas de Sucesso' : 'Measurable Objectives & Success Metrics'}
                        </div>
                        <textarea
                          style={{ ...styles.settingsTextarea, minHeight: '60px' }}
                          value={tapFormData.objectives}
                          onChange={e => setTapFormData({ ...tapFormData, objectives: e.target.value })}
                          placeholder={lang === 'pt' ? 'Metas claras, KPIs...' : 'Clear goals, KPIs...'}
                        />
                      </div>

                      <div style={styles.settingsField}>
                        <div style={styles.settingsLabel}>
                          3. {lang === 'pt' ? 'Descrição Macro e Entregáveis' : 'High-Level Description & Deliverables'}
                        </div>
                        <textarea
                          style={{ ...styles.settingsTextarea, minHeight: '60px' }}
                          value={tapFormData.scope}
                          onChange={e => setTapFormData({ ...tapFormData, scope: e.target.value })}
                          placeholder={lang === 'pt' ? 'O que está no escopo, fases, principais pacotes...' : 'What is in scope, phases, main packages...'}
                        />
                      </div>

                      <div style={styles.settingsField}>
                        <div style={styles.settingsLabel}>
                          4. {lang === 'pt' ? 'Riscos, Premissas e Restrições' : 'Risks, Assumptions & Constraints'}
                        </div>
                        <textarea
                          style={{ ...styles.settingsTextarea, minHeight: '60px' }}
                          value={tapFormData.risks}
                          onChange={e => setTapFormData({ ...tapFormData, risks: e.target.value })}
                          placeholder={lang === 'pt' ? 'Macro riscos identificados, limites de prazo e orçamento...' : 'High level risks identified, budget or time limits...'}
                        />
                      </div>

                      <div style={styles.settingsField}>
                        <div style={styles.settingsLabel}>
                          5. {lang === 'pt' ? 'Partes Interessadas (Stakeholders) e Equipe Core' : 'Key Stakeholders & Core Team'}
                        </div>
                        <textarea
                          style={{ ...styles.settingsTextarea, minHeight: '60px' }}
                          value={tapFormData.stakeholders}
                          onChange={e => setTapFormData({ ...tapFormData, stakeholders: e.target.value })}
                          placeholder={lang === 'pt' ? 'Patrocinador, Gerente, Clientes principais...' : 'Sponsor, Project Manager, Main Clients...'}
                        />
                      </div>

                      <div style={styles.settingsField}>
                        <div style={styles.settingsLabel}>
                          6. {lang === 'pt' ? 'Orçamento e Recursos Necessários' : 'Budget & Resources Allocation'}
                        </div>
                        <textarea
                          style={{ ...styles.settingsTextarea, minHeight: '60px' }}
                          value={tapFormData.budget}
                          onChange={e => setTapFormData({ ...tapFormData, budget: e.target.value })}
                          placeholder={lang === 'pt' ? 'Estimativa financeira de alto nível, equipe dedicada...' : 'High-level financial estimate, dedicated team...'}
                        />
                      </div>
                    </div>
                    <div style={{ ...styles.commandActions, justifyContent: 'flex-end', borderTop: '1px solid var(--border)', padding: '12px 16px', margin: 0 }}>
                      <button style={styles.settingsSecondaryButton} onClick={() => setShowTapModal(false)}>
                        {t('cancel')}
                      </button>
                      <button
                        style={styles.settingsPrimaryButton}
                        disabled={!tapFormData.name.trim()}
                        onClick={async () => {
                          const name = tapFormData.name.trim();
                          if (!name) return;
                          
                          const tapObj = {
                            purpose: tapFormData.purpose,
                            objectives: tapFormData.objectives,
                            scope: tapFormData.scope,
                            risks: tapFormData.risks,
                            stakeholders: tapFormData.stakeholders,
                            budget: tapFormData.budget
                          };

                          if (tapEditingProjectId) {
                            const headers = getAuthHeaders();
                            headers['Content-Type'] = 'application/json';
                            const res = await fetch(`/api/projects/${tapEditingProjectId}`, {
                              method: 'PUT',
                              headers,
                              body: JSON.stringify({ tap_json: JSON.stringify(tapObj) })
                            });
                            if (res.ok) {
                            ...
                            if (pid) {
                              ...
                              await fetch(`/api/projects/${pid}`, {
                                ...
                                headers,
                                body: JSON.stringify({
                                  tap_json: JSON.stringify(tapObj),
                                  expert_mode: 1
                                })
                              });
                      ...
                      >
                        {lang === 'pt' ? 'Salvar Projeto' : 'Save Project'}
                      </button>
                    </div>
                  </div>
                </div>
                  ...
                  <div style={styles.sidebarContent}>
                    <div style={styles.sidebarSection}>
                      ...
                      <button
                        style={{
                          ...styles.sidebarButton,
                          ...(centerView === 'notes' ? styles.sidebarButtonActive : {}),
                          ...(sidebarHoverKey === 'notes' ? styles.sidebarButtonHover : {}),
                          textAlign: sidebarCollapsed ? 'center' : 'left',
                          padding: sidebarCollapsed ? '10px' : styles.sidebarButton.padding,
                          justifyContent: sidebarCollapsed ? 'center' : 'flex-start',
                          marginTop: '8px'
                        }}
                        onMouseEnter={() => setSidebarHoverKey('notes')}
                        onMouseLeave={() => setSidebarHoverKey('')}
                        onClick={() => {
                          setCenterView('notes');
                          setSelectedNote(null);
                        }}
                        title={lang === 'pt' ? 'Notas / Cérebro' : 'Notes / Brain'}
                      >
                        📝{sidebarCollapsed ? '' : ` ${lang === 'pt' ? 'Notas' : 'Notes'}`}
                      </button>
                    </div>
                  </div>
                  ...
                  {centerView === 'conversations' ? (
                      ...
                      {tasksTab === 'project' ? (
                        <div style={styles.lightCard}>
                          ...
                          <div style={{ marginTop: '10px', padding: '10px 12px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: '8px' }}>
                            ...
                            >
                              📋 {lang === 'pt' ? 'Criar com TAP Manualmente (Modo PM)' : 'Create with TAP Manually (PM Mode)'}
                            </button>
                          </div>
                          ...
                          <div style={{ marginTop: '10px', padding: '10px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '8px' }}>
                            ...
                            <div style={{ display: 'grid', gap: '8px', marginTop: '10px' }}>
                              {projects.map((proj) => {
                                ...
                                // Mock de progresso (baseado em globalTodos se houver ligação, senão 0)
                                // Como a ligação projeto<->todo é via conversas, e não temos esse mapa fácil aqui, 
                                // vamos usar um valor simulado ou 0 por enquanto para focar na estética.
                                const percent = isActive ? 15 : 0; // Exemplo: 15% se ativo
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                        ...
                                        >
                                          {isActive ? (lang === 'pt' ? 'Focado' : 'Focused') : (lang === 'pt' ? 'Focar' : 'Focus')}
                                        </button>
                                        <button
                                          style={styles.smallIconButton}
                                          onClick={(e) => { e.stopPropagation(); openRename('project', { id: proj.id, title: proj.name }); }}
                                        >
                                          ✎
                                        </button>
                                        <button
                                    ...
                                    {/* Corpo Expandido */}
                                    {isExpanded && (
                                      <div style={{ padding: '0 16px 16px 40px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                                        
                                        {/* Barra de Progresso */}
                                        <div style={{ marginTop: '12px', marginBottom: '16px' }}>
                                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--text-dim)', marginBottom: '5px' }}>
                                            <span>{lang === 'pt' ? 'Progresso do Escopo' : 'Scope Progress'}</span>
                                            <span>{percent}%</span>
                                          </div>
                                          <div style={{ height: '6px', background: 'var(--bg)', borderRadius: '3px', overflow: 'hidden', border: '1px solid var(--border)' }}>
                                            <div style={{ 
                                              height: '100%', 
                                              width: `${percent}%`, 
                                              background: 'linear-gradient(90deg, var(--accent), #7F77DD)', 
                                        ...
                                        <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                          {lang === 'pt' ? 'Contexto do Projeto' : 'Project Context'}
                                        </div>
                                        <div style={{ 
                                          fontSize: '12px', 
                                          color: 'var(--text)', 
                                          ...
                                          {proj.context_md || (lang === 'pt' ? 'A Sabrina ainda não gerou o contexto para este projeto.' : 'Sabrina has not generated context for this project yet.')}
                                        </div>
                                      </div>
                                    )}
                                  </div>
                          ...
                          {activeProjectId ? (
                            <div style={{ marginTop: '16px', borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
                              {/* Switch Modo Especialista */}
                              {(() => {
                                const activeProject = projects.find(p => Number(p?.id) === Number(activeProjectId));
                                const isExpert = activeProject && (activeProject.expert_mode === 1 || activeProject.expert_mode === true);
                                ...
                                return (
                                  <>
                                    <div style={{
                                      ...
                                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <span style={{ fontSize: '12px', fontWeight: 'bold', color: 'var(--text)' }}>
                                          {lang === 'pt' ? 'Modo Especialista (PMP + Ágil)' : 'Expert Mode (PMP + Agile)'}
                                        </span>
                                        <span style={{ fontSize: '10px', color: 'var(--text-dim)' }}>
                                          {lang === 'pt' ? 'Governança híbrida e entregáveis' : 'Hybrid governance & deliverables'}
                                        </span>
                                      </div>
                                      <button
                                        style={{
                                          background: isExpert ? 'var(--amber)' : 'rgba(255,255,255,0.1)',
                                          color: isExpert ? '#000' : 'var(--text)',
                                          border: 'none',
                                          borderRadius: '6px',
                                          padding: '6px 12px',
                                          fontSize: '11px',
                                        ...
                                        onClick={async () => {
                                          const newMode = isExpert ? 0 : 1;
                                          const headers = getAuthHeaders();
                                          headers['Content-Type'] = 'application/json';
                                          const res = await fetch(`/api/projects/${activeProjectId}`, {
                                            method: 'PUT',
                                            headers,
                                            body: JSON.stringify({ expert_mode: newMode })
                                          });
                                          if (res.ok) {
                                            await loadProjects();
                                          }
                                    ...
                                    {/* Dashboard Modo Especialista se ativo */}
                                    {isExpert ? (
                                      <div style={{
                                        ...
                                        <div style={{ fontSize: '13px', fontWeight: 'bold', color: 'var(--amber)', marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                          <span>🏆 {lang === 'pt' ? 'Governança de Projeto Híbrida' : 'Hybrid Project Governance'}</span>
                                          <button
                                            style={{ ...styles.settingsSecondaryButton, padding: '3px 8px', fontSize: '10px' }}
                                            onClick={() => {
                                              setTapEditingProjectId(activeProjectId);
                                              let existingTap = {};
                                              try { existingTap = JSON.parse(activeProject?.tap_json || '{}'); } catch { existingTap = {}; }
                                              setTapFormData({
                                                name: activeProject?.name || '',
                                                purpose: existingTap.purpose || '',
                                                objectives: existingTap.objectives || '',
                                                scope: existingTap.scope || '',
                                                risks: existingTap.risks || '',
                                                stakeholders: existingTap.stakeholders || '',
                                                budget: existingTap.budget || ''
                                              });
                                              setShowTapModal(true);
                                            }}
                                          >
                                            📝 {lang === 'pt' ? 'Editar TAP' : 'Edit TAP'}
                                          </button>
                                        </div>

                                        {/* Abas do Painel Expert */}
                                        <div style={{ display: 'flex', gap: '4px', borderBottom: '1px solid var(--border)', paddingBottom: '8px', marginBottom: '12px', overflowX: 'auto', whiteSpace: 'nowrap' }}>
                                          {[
                                            { id: 'tap', label: 'TAP' },
                                            { id: 'kanban', label: lang === 'pt' ? 'Kanban' : 'Kanban' },
                                            { id: 'metrics', label: lang === 'pt' ? 'Métricas & Curva S' : 'Metrics & S-Curve' },
                                            { id: 'stakeholders', label: lang === 'pt' ? 'Stakeholders' : 'Stakeholders' },
                                            { id: 'risks_roadmap', label: lang === 'pt' ? 'Riscos & Roadmap' : 'Risks & Roadmap' },
                                            { id: 'lessons', label: lang === 'pt' ? 'Retrospectiva' : 'Retro' }
                                          ].map(tab => (
                                            <button
                                              key={tab.id}
                                              onClick={() => setExpertTab(tab.id)}
                                              style={{
                                                background: expertTab === tab.id ? 'rgba(255,255,255,0.08)' : 'transparent',
                                                border: 'none',
                                                borderRadius: '4px',
                                                color: expertTab === tab.id ? 'var(--text-bright)' : 'var(--text-dim)',
                                                fontSize: '11px',
                                                padding: '4px 8px',
                                                cursor: 'pointer'
                                              }}
                                        ...
                                        <div style={{ minHeight: '180px' }}>
                                          {expertTab === 'tap' && (() => {
                                            let tap = {};
                                            try { tap = JSON.parse(activeProject?.tap_json || '{}'); } catch { tap = {}; }
                                            const hasTap = tap.purpose || tap.objectives || tap.scope || tap.risks || tap.stakeholders || tap.budget;

                                            if (!hasTap) {
                                              return (
                                                <div style={{ fontSize: '11px', color: 'var(--text-dim)', fontStyle: 'italic', padding: '12px', textAlign: 'center' }}>
                                                  {lang === 'pt'
                                                    ? 'Nenhum Termo de Abertura (TAP) preenchido. Clique em "Editar TAP" ou descreva o projeto no chat para que os agentes gerem.'
                                                    : 'No Project Charter (TAP) populated yet. Click "Edit TAP" or describe the project in the chat to let agents generate it.'}
                                                </div>
                                              );
                                            }
                                            ...
                                            return (
                                              <div style={{ display: 'grid', gap: '10px', fontSize: '11px' }}>
                                                <div>
                                                  <strong style={{ color: 'var(--text-bright)' }}>1. {lang === 'pt' ? 'Propósito e Justificativa' : 'Purpose & Business Case'}:</strong>
                                                  <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', marginTop: '3px', whiteSpace: 'pre-wrap' }}>{tap.purpose || '-'}</div>
                                                </div>
                                                <div>
                                                  <strong style={{ color: 'var(--text-bright)' }}>2. {lang === 'pt' ? 'Objetivos Mensuráveis e Métricas de Sucesso' : 'Measurable Objectives & Success Metrics'}:</strong>
                                                  <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', marginTop: '3px', whiteSpace: 'pre-wrap' }}>{tap.objectives || '-'}</div>
                                                </div>
                                                <div>
                                                  <strong style={{ color: 'var(--text-bright)' }}>3. {lang === 'pt' ? 'Descrição Macro e Entregáveis' : 'High-Level Description & Deliverables'}:</strong>
                                                  <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', marginTop: '3px', whiteSpace: 'pre-wrap' }}>{tap.scope || '-'}</div>
                                                </div>
                                                <div>
                                                  <strong style={{ color: 'var(--text-bright)' }}>4. {lang === 'pt' ? 'Riscos, Premissas e Restrições' : 'Riscos, Assumptions & Constraints'}:</strong>
                                                  <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', marginTop: '3px', whiteSpace: 'pre-wrap' }}>{tap.risks || '-'}</div>
                                                </div>
                                                <div>
                                                  <strong style={{ color: 'var(--text-bright)' }}>5. {lang === 'pt' ? 'Partes Interessadas e Equipe' : 'Stakeholders & Team Core'}:</strong>
                                                  <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', marginTop: '3px', whiteSpace: 'pre-wrap' }}>{tap.stakeholders || '-'}</div>
                                                </div>
                                                <div>
                                                  <strong style={{ color: 'var(--text-bright)' }}>6. {lang === 'pt' ? 'Orçamento e Recursos' : 'Budget & Resources'}:</strong>
                                                  <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', marginTop: '3px', whiteSpace: 'pre-wrap' }}>{tap.budget || '-'}</div>
                                                </div>
                                              </div>
                                            );
                                          })()}
                                          ...
                                          {expertTab === 'kanban' && (() => {
                                            const projTodos = globalTodos.filter(t => t.project_id === activeProjectId || t.project_id === String(activeProjectId));
                                            const todoCol = projTodos.filter(t => !t.done && t.status !== 'in_progress');
                                            const inProgressCol = projTodos.filter(t => !t.done && t.status === 'in_progress');
                                            const doneCol = projTodos.filter(t => t.done);

                                            return (
                                              <div>
                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                                                  {/* Coluna A Fazer */}
                                                  <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '6px', padding: '6px' }}>
                                                    <div style={{ fontSize: '10px', fontWeight: 'bold', color: 'var(--text-dim)', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '3px', marginBottom: '6px' }}>
                                                      📋 {lang === 'pt' ? 'A Fazer' : 'To Do'} ({todoCol.length})
                                                    </div>
                                                    <div style={{ display: 'grid', gap: '4px', maxHeight: '180px', overflowY: 'auto' }}>
                                                      {todoCol.map(t => (
                                                        <div key={t.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '4px', padding: '6px', fontSize: '10px' }}>
                                                          <div style={{ color: 'var(--text)' }}>{t.title}</div>
                                                          <button
                                                            style={{ border: 'none', background: 'rgba(255,255,255,0.05)', color: 'var(--accent)', cursor: 'pointer', marginTop: '4px', fontSize: '9px', borderRadius: '2px', padding: '1px 4px' }}
                                                            onClick={async () => {
                                                              const headers = getAuthHeaders();
                                                              headers['Content-Type'] = 'application/json';
                                                              await fetch(`/api/todos/${t.id}`, { method: 'PUT', headers, body: JSON.stringify({ status: 'in_progress' }) });
                                                              await loadTodos();
                                                            }}
                                                          >
                                                  ...
                                                  {/* Coluna Em Andamento */}
                                                  <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '6px', padding: '6px' }}>
                                                    <div style={{ fontSize: '10px', fontWeight: 'bold', color: 'var(--amber)', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '3px', marginBottom: '6px' }}>
                                                      ⚡ {lang === 'pt' ? 'Em Progresso' : 'In Progress'} ({inProgressCol.length})
                                                    </div>
                                                    <div style={{ display: 'grid', gap: '4px', maxHeight: '180px', overflowY: 'auto' }}>
                                                      {inProgressCol.map(t => (
                                                        <div key={t.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '4px', padding: '6px', fontSize: '10px' }}>
                                                          <div style={{ color: 'var(--text)' }}>{t.title}</div>
                                                          <button
                                                            style={{ border: 'none', background: 'rgba(255,255,255,0.05)', color: 'var(--green)', cursor: 'pointer', marginTop: '4px', fontSize: '9px', borderRadius: '2px', padding: '1px 4px' }}
                                                            onClick={async () => {
                                                              const headers = getAuthHeaders();
                                                              headers['Content-Type'] = 'application/json';
                                                              await fetch(`/api/todos/${t.id}`, { method: 'PUT', headers, body: JSON.stringify({ done: true, status: 'completed' }) });
                                                              await loadTodos();
                                                            }}
                                                          >
                                                  ...
                                                  {/* Coluna Concluído */}
                                                  <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '6px', padding: '6px' }}>
                                                    <div style={{ fontSize: '10px', fontWeight: 'bold', color: 'var(--green)', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '3px', marginBottom: '6px' }}>
                                                      ✓ {lang === 'pt' ? 'Concluído' : 'Done'} ({doneCol.length})
                                                    </div>
                                                    <div style={{ display: 'grid', gap: '4px', maxHeight: '180px', overflowY: 'auto' }}>
                                                      {doneCol.map(t => (
                                                        <div key={t.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '4px', padding: '6px', fontSize: '10px', opacity: 0.6 }}>
                                                          <div style={{ color: 'var(--text)', textDecoration: 'line-through' }}>{t.title}</div>
                                                        </div>
                                                      ))}
                                                    </div>
                                                  </div>
                                          ...
                                          {expertTab === 'metrics' && (() => {
                                            const projTodos = globalTodos.filter(t => t.project_id === activeProjectId || t.project_id === String(activeProjectId));
                                            const totalCount = projTodos.length || 5;
                                            const doneCount = projTodos.filter(t => t.done).length;
                                            
                                            const pv = 80;
                                            const ev = Math.round((doneCount / totalCount) * 100);
                                            const ac = ev + 5;
                                            const spi = Number(pv > 0 ? (ev / pv).toFixed(2) : 1.0);
                                            const cpi = Number(ac > 0 ? (ev / ac).toFixed(2) : 1.0);

                                            return (
                                              <div>
                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '16px' }}>
                                                  <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '6px', padding: '8px', textAlign: 'center' }}>
                                                    <div style={{ fontSize: '9px', color: 'var(--text-dim)', textTransform: 'uppercase' }}>SPI (Cronograma)</div>
                                                    <div style={{ fontSize: '16px', fontWeight: 'bold', color: spi >= 1.0 ? 'var(--green)' : 'var(--red)' }}>{spi}</div>
                                                    <div style={{ fontSize: '8px', color: 'var(--text-dim)' }}>
                                                      {spi >= 1.0 ? (lang === 'pt' ? 'No prazo' : 'On track') : (lang === 'pt' ? 'Atrasado' : 'Delayed')}
                                                    </div>
                                                  </div>
                                                  <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '6px', padding: '8px', textAlign: 'center' }}>
                                                    <div style={{ fontSize: '9px', color: 'var(--text-dim)', textTransform: 'uppercase' }}>CPI (Custo)</div>
                                                    <div style={{ fontSize: '16px', fontWeight: 'bold', color: cpi >= 1.0 ? 'var(--green)' : 'var(--red)' }}>{cpi}</div>
                                                    <div style={{ fontSize: '8px', color: 'var(--text-dim)' }}>
                                                      {cpi >= 1.0 ? (lang === 'pt' ? 'Sob orçamento' : 'Under budget') : (lang === 'pt' ? 'Estouro de custo' : 'Over budget')}
                                                    </div>
                                                  </div>
                                                  <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '6px', padding: '8px', textAlign: 'center' }}>
                                                    <div style={{ fontSize: '9px', color: 'var(--text-dim)', textTransform: 'uppercase' }}>Progresso Híbrido</div>
                                                    <div style={{ fontSize: '16px', fontWeight: 'bold', color: 'var(--accent)' }}>{ev}%</div>
                                                    <div style={{ fontSize: '8px', color: 'var(--text-dim)' }}>{doneCount}/{totalCount} {lang === 'pt' ? 'Tarefas' : 'Tasks'}</div>
                                                  </div>
                                                </div>

                                                <div style={{ fontSize: '10px', color: 'var(--text-dim)', marginBottom: '4px' }}>
                                                  📈 {lang === 'pt' ? 'Curva S do Projeto (PV vs EV vs AC)' : 'Project S-Curve (PV vs EV vs AC)'}
                                                </div>
                                                <div style={{ width: '100%', height: '80px', background: 'rgba(0,0,0,0.2)', borderRadius: '6px', overflow: 'hidden', padding: '4px' }}>
                                                  <svg width="100%" height="100%" viewBox="0 0 300 80" preserveAspectRatio="none">
                                                    ...
                                                    <path d="M 0,80 Q 75,70 150,40 T 300,0" fill="none" stroke="var(--amber)" strokeWidth="1.5" strokeDasharray="3,3" />
                                                    <path d={`M 0,80 Q 75,75 150,${80 - ev*0.6} T 300,${80 - ev*0.8}`} fill="none" stroke="var(--green)" strokeWidth="2" />
                                                    <path d={`M 0,80 Q 75,72 150,${80 - ac*0.58} T 300,${80 - ac*0.78}`} fill="none" stroke="var(--red)" strokeWidth="1" />
                                                  </svg>
                                                  <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', fontSize: '8px', color: 'var(--text-dim)', marginTop: '2px' }}>
                                                    <span><span style={{ color: 'var(--amber)' }}>---</span> Planned Value</span>
                                                    <span><span style={{ color: 'var(--green)' }}>──</span> Earned Value</span>
                                                    <span><span style={{ color: 'var(--red)' }}>──</span> Actual Cost</span>
                                                  </div>
                                                </div>
                                              </div>
                                            );
                                          ...
                                          {expertTab === 'stakeholders' && (() => {
                                            let list = [];
                                            try { list = JSON.parse(activeProject?.stakeholders_json || '[]'); } catch { list = []; }

                                            return (
                                              <div>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                                  <span style={{ fontSize: '10px', color: 'var(--text-dim)', fontWeight: 'bold' }}>
                                                    {lang === 'pt' ? 'Mapeamento de Partes Interessadas' : 'Stakeholders Mapping'}
                                                  </span>
                                                  <button
                                                    style={{ ...styles.settingsSecondaryButton, padding: '2px 6px', fontSize: '9px' }}
                                                    onClick={() => setShowAddStakeholder(!showAddStakeholder)}
                                                  >
                                                    {showAddStakeholder ? (lang === 'pt' ? 'Cancelar' : 'Cancel') : `+ ${lang === 'pt' ? 'Adicionar' : 'Add'}`}
                                                  </button>
                                                </div>

                                                {showAddStakeholder && (
                                                  <div style={{ display: 'grid', gap: '6px', background: 'rgba(0,0,0,0.15)', padding: '8px', borderRadius: '6px', marginBottom: '10px' }}>
                                                    <input
                                                      style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px' }}
                                                      placeholder={lang === 'pt' ? 'Nome do stakeholder' : 'Stakeholder name'}
                                                      value={stakeholderForm.name}
                                                      onChange={e => setStakeholderForm({ ...stakeholderForm, name: e.target.value })}
                                                    />
                                                    <input
                                                      style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px' }}
                                                      placeholder={lang === 'pt' ? 'Papel (Ex: Sponsor, PO, Dev)' : 'Role (Ex: Sponsor, PO, Dev)'}
                                                      value={stakeholderForm.role}
                                                      onChange={e => setStakeholderForm({ ...stakeholderForm, role: e.target.value })}
                                                    />
                                                    <div style={{ display: 'flex', gap: '6px' }}>
                                                      <select
                                                        style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px', flex: 1 }}
                                                        value={stakeholderForm.impact}
                                                        onChange={e => setStakeholderForm({ ...stakeholderForm, impact: e.target.value })}
                                                      >
                                                      ...
                                                      <input
                                                        style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px', flex: 1 }}
                                                        placeholder={lang === 'pt' ? 'Comunicação (Ex: Slack)' : 'Communication'}
                                                        value={stakeholderForm.channel}
                                                        onChange={e => setStakeholderForm({ ...stakeholderForm, channel: e.target.value })}
                                                      />
                                                    </div>
                                                    <button
                                                      style={{ ...styles.settingsPrimaryButton, padding: '4px 8px', fontSize: '10px' }}
                                                ...
                                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
                                                  <thead>
                                                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', textAlign: 'left', color: 'var(--text-dim)' }}>
                                                      <th style={{ padding: '4px' }}>{lang === 'pt' ? 'Nome' : 'Name'}</th>
                                                      <th style={{ padding: '4px' }}>{lang === 'pt' ? 'Papel' : 'Role'}</th>
                                                      <th style={{ padding: '4px' }}>{lang === 'pt' ? 'Impacto' : 'Impact'}</th>
                                                      <th style={{ padding: '4px' }}>{lang === 'pt' ? 'Comunicação' : 'Comm'}</th>
                                                      <th style={{ padding: '4px', textAlign: 'right' }}>Ação</th>
                                                    </tr>
                                                  </thead>
                                                  <tbody>
                                                    {list.map(s => (
                                                      <tr key={s.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                                                        <td style={{ padding: '4px', color: 'var(--text-bright)' }}>{s.name}</td>
                                                        <td style={{ padding: '4px' }}>{s.role}</td>
                                                        <td style={{ padding: '4px', color: s.impact === 'High' ? 'var(--red)' : (s.impact === 'Medium' ? 'var(--amber)' : 'var(--text-dim)') }}>{s.impact}</td>
                                                        <td style={{ padding: '4px' }}>{s.channel}</td>
                                                        <td style={{ padding: '4px', textAlign: 'right' }}>
                                                          <button style={{ border: 'none', background: 'transparent', color: 'var(--red)', cursor: 'pointer', fontSize: '11px' }} onClick={() => handleDeleteStakeholder(s.id)}>×</button>
                                                        </td>
                                                      </tr>
                                                    ))}
                                                    {list.length === 0 ? (
                                                      <tr>
                                                        <td colSpan="5" style={{ padding: '8px', textAlign: 'center', color: 'var(--text-dim)', fontStyle: 'italic' }}>
                                                          {lang === 'pt' ? 'Nenhuma parte interessada cadastrada.' : 'No stakeholders registered.'}
                                                        </td>
                                                      </tr>
                                                    ) : null}
                                                  </tbody>
                                          ...
                                          {expertTab === 'risks_roadmap' && (() => {
                                            let risksList = [];
                                            try { risksList = JSON.parse(activeProject?.risk_matrix_json || '[]'); } catch { risksList = []; }

                                            let releasesList = [];
                                            try { releasesList = JSON.parse(activeProject?.release_plan_json || '[]'); } catch { releasesList = []; }

                                            return (
                                              <div>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                                                  <span style={{ fontSize: '10px', color: 'var(--amber)', fontWeight: 'bold' }}>⚠️ Matriz de Riscos</span>
                                                  <button
                                                    style={{ ...styles.settingsSecondaryButton, padding: '1px 5px', fontSize: '9px' }}
                                                    onClick={() => setShowAddRisk(!showAddRisk)}
                                                  >
                                                    {showAddRisk ? 'Cancelar' : '+ Adicionar'}
                                                  </button>
                                                </div>
                                                  ...
                                                  <div style={{ display: 'grid', gap: '6px', background: 'rgba(0,0,0,0.15)', padding: '8px', borderRadius: '6px', marginBottom: '10px' }}>
                                                    ...
                                                    <div style={{ display: 'flex', gap: '6px' }}>
                                                      ...
                                                      >
                                                        ...
                                                        <option value="Medium">Prob: Média</option>
                                                        <option value="Low">Prob: Baixa</option>
                                                      </select>
                                                      <select
                                                        style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px', flex: 1 }}
                                                      ...
                                                      >
                                                        <option value="High">Imp: Alto</option>
                                                        <option value="Medium">Imp: Médio</option>
                                                        <option value="Low">Imp: Baixo</option>
                                                      </select>
                                                    </div>
                                                    <input
                                                      style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px' }}
                                                ...
                                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '9px', marginBottom: '12px' }}>
                                                  <thead>
                                                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', textAlign: 'left', color: 'var(--text-dim)' }}>
                                                      <th style={{ padding: '4px' }}>Risco</th>
                                                      <th style={{ padding: '4px' }}>Prob / Imp</th>
                                                      <th style={{ padding: '4px' }}>Mitigação</th>
                                                      <th style={{ padding: '4px', textAlign: 'right' }}>Ação</th>
                                                    </tr>
                                                  </thead>
                                                  <tbody>
                                                    {risksList.map(r => (
                                                      <tr key={r.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                                                        <td style={{ padding: '4px', color: 'var(--text-bright)' }}>{r.description}</td>
                                                        <td style={{ padding: '4px' }}>{r.probability} / {r.impact}</td>
                                                        <td style={{ padding: '4px' }}>{r.mitigation}</td>
                                                        <td style={{ padding: '4px', textAlign: 'right' }}>
                                                          <button style={{ border: 'none', background: 'transparent', color: 'var(--red)', cursor: 'pointer', fontSize: '11px' }} onClick={() => handleDeleteRisk(r.id)}>×</button>
                                                        </td>
                                                      </tr>
                                                    ))}
                                                    {risksList.length === 0 ? (
                                                      <tr>
                                                        <td colSpan="4" style={{ padding: '6px', textAlign: 'center', color: 'var(--text-dim)', fontStyle: 'italic' }}>
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                                                  <span style={{ fontSize: '10px', color: 'var(--accent)', fontWeight: 'bold' }}>📅 Plano de Releases / Roadmap</span>
                                                  <button
                                                    style={{ ...styles.settingsSecondaryButton, padding: '1px 5px', fontSize: '9px' }}
                                                    onClick={() => setShowAddRelease(!showAddRelease)}
                                                  >
                                                    {showAddRelease ? 'Cancelar' : '+ Adicionar'}
                                                  </button>
                                                </div>
                                                ...
                                                {showAddRelease && (
                                                  <div style={{ display: 'grid', gap: '6px', background: 'rgba(0,0,0,0.15)', padding: '8px', borderRadius: '6px', marginBottom: '10px' }}>
                                                    <input
                                                      style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px' }}
                                                      placeholder="Nome da Release / Sprint"
                                                    ...
                                                    <div style={{ display: 'flex', gap: '6px' }}>
                                                      <input
                                                        style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px', flex: 1 }}
                                                        placeholder="Data estimada (Ex: Q3 2026)"
                                                    ...
                                                    <input
                                                      style={{ ...styles.settingsInput, padding: '4px 6px', fontSize: '10px' }}
                                                      placeholder="Escopo / Principais entregas"
                                                      value={releaseForm.scope}
                                                      onChange={e => setReleaseForm({ ...releaseForm, scope: e.target.value })}
                                                    />
                                                    <button
                                                      style={{ ...styles.settingsPrimaryButton, padding: '4px 8px', fontSize: '10px' }}
                                                ...
                                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '9px' }}>
                                                  <thead>
                                                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', textAlign: 'left', color: 'var(--text-dim)' }}>
                                                      <th style={{ padding: '4px' }}>Release</th>
                                                      <th style={{ padding: '4px' }}>Data</th>
                                                      <th style={{ padding: '4px' }}>Escopo</th>
                                                  ...
                                                  <tbody>
                                                    {releasesList.map(r => (
                                                      <tr key={r.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                                                        <td style={{ padding: '4px', color: 'var(--text-bright)' }}>{r.name}</td>
                                                        <td style={{ padding: '4px' }}>{r.date}</td>
                                                        <td style={{ padding: '4px' }}>{r.scope}</td>
                                                        <td style={{ padding: '4px', color: r.status === 'Concluído' ? 'var(--green)' : (r.status === 'Em Andamento' ? 'var(--amber)' : 'var(--text-dim)') }}>{r.status}</td>
                                                        <td style={{ padding: '4px', textAlign: 'right' }}>
                                                          <button style={{ border: 'none', background: 'transparent', color: 'var(--red)', cursor: 'pointer', fontSize: '11px' }} onClick={() => handleDeleteRelease(r.id)}>×</button>
                                                        </td>
                                                      </tr>
                                                    ))}
                                                    {releasesList.length === 0 ? (
                                                      <tr>
                                                        <td colSpan="5" style={{ padding: '6px', textAlign: 'center', color: 'var(--text-dim)', fontStyle: 'italic' }}>
                                                          Nenhum plano de liberação cadastrado.
                                                        </td>
                                                      </tr>
                                                    ) : null}
                                                  </tbody>
                                          ...
                                          {expertTab === 'lessons' && (() => {
                                            let tap = {};
                                            try { tap = JSON.parse(activeProject?.tap_json || '{}'); } catch { tap = {}; }
                                            let retro = tap.lessons || '';

                                            return (
                                              <div>
                                                <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '8px' }}>
                                                  📝 {lang === 'pt' ? 'Retrospectiva & Lições Aprendidas' : 'Retro & Lessons Learned'}
                                                </div>
                                                <textarea
                                                  style={{ ...styles.settingsTextarea, minHeight: '100px', fontSize: '11px' }}
                                                  placeholder={lang === 'pt' ? 'Registre os aprendizados do projeto para evitar erros no futuro...' : 'Log project learnings to avoid mistakes in future runs...'}
                                                  value={retro}
                                                  onChange={async (e) => {
                                                    const updatedTap = { ...tap, lessons: e.target.value };
                                                    const headers = getAuthHeaders();
                                                    headers['Content-Type'] = 'application/json';
                                                    await fetch(`/api/projects/${activeProjectId}`, {
                                                      method: 'PUT',
                                                      headers,
                                                      body: JSON.stringify({ tap_json: JSON.stringify(updatedTap) })
                                                    });
                                                    activeProject.tap_json = JSON.stringify(updatedTap);
                                                  }}
                                                />
                                              </div>
                                            );
                    ...
                    <div style={{ ...styles.centerPanel, display: 'flex', flexDirection: 'column', height: '100%' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                        <div style={styles.centerPanelTitle}>{lang === 'pt' ? 'Notas & Ideias' : 'Notes & Ideas'}</div>
                        <button
                          style={styles.settingsPrimaryButton}
                          onClick={() => setSelectedNote({ title: '', content_md: '', tags: '', project_id: '' })}
                        >
                          + {lang === 'pt' ? 'Nova Nota' : 'New Note'}
                        </button>
                      </div>
                      ...
                      <div style={{ display: 'flex', flex: 1, gap: '20px', minHeight: 0 }}>
                        ...
                        <div style={{ width: '320px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                          <input
                            style={styles.settingsInput}
                            placeholder={lang === 'pt' ? 'Buscar notas...' : 'Search notes...'}
                          ...
                          <select
                            style={styles.settingsInput}
                            value={notesFilterProjectId}
                            onChange={(e) => setNotesFilterProjectId(e.target.value)}
                          >
                            <option value="all">{lang === 'pt' ? 'Todos os projetos' : 'All projects'}</option>
                            <option value="none">{lang === 'pt' ? 'Sem projeto' : 'No project'}</option>
                            {projects.map(p => (
                          ...
                          <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px', paddingRight: '4px' }}>
                            {notesLoading ? (
                              <div style={{ textAlign: 'center', color: 'var(--text-dim)', padding: '20px', fontFamily: 'var(--mono)' }}>
                                {lang === 'pt' ? 'Carregando...' : 'Loading...'}
                              </div>
                            ) : notes.length === 0 ? (
                              <div style={{ textAlign: 'center', color: 'var(--text-dim)', padding: '20px' }}>
                                {lang === 'pt' ? 'Nenhuma nota encontrada' : 'No notes found'}
                              </div>
                            ) : (() => {
                              const filtered = notes.filter(n => {
                                ...
                                const matchesSearch = !noteSearchQuery || 
                                  ...
                                  (n.content_md || '').toLowerCase().includes(noteSearchQuery.toLowerCase());
                                
                                // Filter by project
                                let matchesProject = true;
                                if (notesFilterProjectId === 'none') {
                                  matchesProject = !n.project_id;
                                } else if (notesFilterProjectId !== 'all') {
                                  matchesProject = String(n.project_id) === String(notesFilterProjectId);
                                }
                              ...
                              if (filtered.length === 0) {
                                return (
                                  <div style={{ textAlign: 'center', color: 'var(--text-dim)', padding: '20px' }}>
                                    {lang === 'pt' ? 'Nenhuma nota corresponde aos filtros' : 'No notes match the filters'}
                                  </div>
                                );
                              }
                              ...
                              return filtered.map(n => {
                                ...
                                return (
                                  ...
                                  >
                                    <div style={{ fontWeight: 600, color: isSelected ? 'var(--accent)' : 'var(--text-bright)', fontSize: '14px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                      {n.title || (lang === 'pt' ? 'Sem título' : 'Untitled')}
                                    </div>
                                    <div style={{ fontSize: '11px', color: 'var(--text-dim)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                      {n.content_md ? n.content_md.slice(0, 60) : '...'}
                                    </div>
                                    {n.tags && (
                                      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '4px' }}>
                                        {n.tags.split(',').map((t, idx) => (
                                          <span key={idx} style={{ fontSize: '10px', background: 'rgba(255,255,255,0.06)', padding: '2px 6px', borderRadius: '4px', color: 'var(--text-dim)', border: '1px solid var(--border)' }}>
                                            {t.trim()}
                                          </span>
                                        ))}
                                      </div>
                        ...
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-panel)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px', minWidth: 0 }}>
                          {selectedNote ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', height: '100%' }}>
                              {/* Title */}
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                <label style={{ fontSize: '12px', color: 'var(--text-dim)', fontFamily: 'var(--mono)' }}>
                                  {lang === 'pt' ? 'TÍTULO' : 'TITLE'}
                                </label>
                                <input
                                  style={{ ...styles.settingsInput, fontSize: '16px', fontWeight: 600 }}
                                  value={selectedNote.title || ''}
                                  placeholder={lang === 'pt' ? 'Título da nota...' : 'Note title...'}
                                  onChange={(e) => setSelectedNote({ ...selectedNote, title: e.target.value })}
                                />
                              </div>

                              {/* Meta: Project and Tags */}
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                  <label style={{ fontSize: '12px', color: 'var(--text-dim)', fontFamily: 'var(--mono)' }}>
                                    {lang === 'pt' ? 'ASSOCIAR AO PROJETO' : 'ASSOCIATE WITH PROJECT'}
                                  </label>
                                  <select
                                    style={styles.settingsInput}
                                    value={selectedNote.project_id || ''}
                                    onChange={(e) => setSelectedNote({ ...selectedNote, project_id: e.target.value })}
                                  >
                                    <option value="">{lang === 'pt' ? 'Nenhum projeto' : 'No project'}</option>
                                    {projects.map(p => (
                                      <option key={p.id} value={p.id}>{p.name}</option>
                                    ))}
                                  </select>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                  ...
                                  <input
                                    ...
                                    placeholder="idea, research, logic..."
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flex: 1, minHeight: 0 }}>
                                <label style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: 'var(--text-dim)', fontFamily: 'var(--mono)' }}>
                                  <span>{lang === 'pt' ? 'CONTEÚDO (MARKDOWN)' : 'CONTENT (MARKDOWN)'}</span>
                                  <span>Markdown</span>
                                </label>
                                <textarea
                                  style={{
                                    ...styles.settingsTextarea,
                                    flex: 1,
                                    resize: 'none',
                                    fontFamily: 'var(--mono)',
                                    fontSize: '13px',
                                  ...
                                  placeholder={lang === 'pt' ? 'Escreva suas ideias aqui...' : 'Write your ideas here...'}
                                  onChange={(e) => setSelectedNote({ ...selectedNote, content_md: e.target.value })}
                                />
                              </div>
                              ...
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
                                {selectedNote.id ? (
                                  ...
                                  >
                                    {lang === 'pt' ? 'Excluir Nota' : 'Delete Note'}
                                  </button>
                                ) : <div />}
                                ...
                                <div style={{ display: 'flex', gap: '12px' }}>
                                  ...
                                  <button
                                    ...
                                    disabled={noteSaving}
                                  >
                                    {noteSaving ? '...' : (lang === 'pt' ? 'Salvar Nota' : 'Save Note')}
                                  </button>
                                </div>
                              </div>
                            </div>
                            ...
                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-dim)', gap: '12px' }}>
                              ...
                              <div style={{ fontSize: '15px', fontWeight: 500 }}>
                                {lang === 'pt' ? 'Selecione uma nota para visualizar' : 'Select a note to view'}
                              </div>
                              <div style={{ fontSize: '13px', textAlign: 'center', maxWidth: '300px', lineHeight: 1.5 }}>
                                {lang === 'pt'
                                  ? 'Clique em uma nota na lista lateral ou crie uma nova nota para começar a escrever.'
                                  : 'Click on a note in the list or create a new note to start writing.'}
                              </div>
                            </div>
                          )}
                        </div>
                    ...
                    <div style={styles.centerPanel}>
                      ...
                      {settingsTab === 'system' ? (
                        <SystemSettingsPanel
                          ...
                          planAutoApproveInformational={planAutoApproveInformational}
                          planAutoApproveAll={planAutoApproveAll}
                          onPlanAutoApproveInformationalChange={(v) => {
                            setPlanAutoApproveInformational(v);
                            localStorage.setItem('planAutoApproveInformational', String(v));
                          }}
                    ...
                    <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
                      ...
                      <div style={styles.messagesContainer}>
                        ...
                        {(chatSearch.trim()
                          ...
                          : messages
                        ).filter(m => !(m.role === 'user' && (m.content || '').includes('WIZARD_ID:'))).map(message => (
                          <div
                            key={message.id}
                            style={{
                              ...styles.message,
                          ...
                          >
                            ...
                            <div style={styles.messageContent}>
                              <div
                                style={{
                                  ...(message.role === 'user' ? styles.messageBubble : styles.messageBubbleAssistant),
                                ...
                                {message.role === 'assistant' && !message.streaming ? (
                                  (() => {
                                    ...
                                    if (message?.cli_runs && typeof message.cli_runs === 'object') {
                                     ...
                                     if (!(blocks?.length > 0 || parsed.requests?.length > 0 || cliBlocks.length > 0) && message.content) {
                    ...
                    {currentConversation && currentKind === 'task' && (
                      <div style={styles.sidebarRight}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                          <div style={{ fontSize: '12px', color: 'var(--text-bright)', fontFamily: 'var(--mono)', fontWeight: 'bold', letterSpacing: '0.05em' }}>
                            {t('todos').toUpperCase()}
                          </div>
                          <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                            <button
                              style={{ ...styles.settingsSecondaryButton, padding: '4px 8px', fontSize: '10px' }}
                              onClick={() => setShowTaskDoneTodos((v) => !v)}
                              title={showTaskDoneTodos ? t('hide_done') : t('show_done')}
                            >
                              {showTaskDoneTodos ? '👁️' : '🕶️'}
                            </button>
                            <button
                              style={{ ...styles.settingsSecondaryButton, padding: '4px 8px', fontSize: '10px' }}
                        ...
                        <div style={{ marginBottom: '16px', display: 'flex', gap: '6px' }}>
                          <input
                            style={{ ...styles.settingsInput, flex: 1, padding: '8px 10px', fontSize: '11px' }}
                            value={taskTodoDraft}
                          ...
                          />
                          <button
                            style={{ ...styles.settingsPrimaryButton, padding: '8px 12px', fontSize: '11px' }}
                        ...
                        <div style={{ display: 'grid', gap: '8px' }}>
                                ...
                                <div key={node.id} style={{ display: 'grid', gap: '6px' }}>
                                  <div
                                    style={{
                                      ...
                                      border: '1px solid var(--border)',
                                      background: 'var(--bg3)',
                                      marginLeft: `${Math.min(4, Math.max(0, depth)) * 10}px`
                                    }}
                                  >
                                    <button
                                      style={{
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                      ...
                                      {(deliveryPath || (artifacts && artifacts.length)) ? (
                                        <div style={{ marginTop: '4px', display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                          {deliveryPath ? (
                                            <span style={{ fontSize: '9px', padding: '1px 6px', borderRadius: '999px', background: 'rgba(245,166,35,0.10)', color: 'var(--amber)', fontFamily: 'var(--mono)' }} title={deliveryPath}>
                                              📍
                                            </span>
                                          ) : null}
                                          {(artifacts || []).slice(0, 3).map((a, i) => (
                                            <button
                                              key={i}
                                              style={{ ...styles.settingsSecondaryButton, padding: '2px 6px', fontSize: '9px', fontFamily: 'var(--mono)' }}
                          ...
                          {(!taskTodos || taskTodos.length === 0) ? (
                            <div style={{ fontSize: '11px', color: 'var(--text-dim)', fontFamily: 'var(--mono)', textAlign: 'center', padding: '20px 0' }}>
                              {t('no_todos')}
                            </div>
                          ) : null}
                        </div>
