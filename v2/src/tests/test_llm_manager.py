...
class TestOllamaProvider:
    ...
    async def test_validate_connection_success(self, provider):
        with patch('aiohttp.ClientSession.get') as mock_get:
            ...
            mock_session.status = 200
            mock_session.json.return_value = {"models": [{"name": "llama2"}]}
...
class TestLLMManagerIntegration:
    ...
    async def test_ollama_integration(self):
        ...
        try:
            ...
            async with session.get(f"{provider.base_url}/api/tags") as response:
                data = await response.json()
                models = data.get("models") or []
                model_name = (models[0] or {}).get("name") if models else None

            if not model_name:
                pytest.skip("No Ollama models installed")

            response = await provider.generate("Hello", model=model_name)
