...
class TestNewTablesExist:
    def test_message_feedback_table(self, tmp_path):
        ...
        mgr = DatabaseManager(str(tmp_path / "test.db"))
    ...
    def test_soul_events_has_salience(self, tmp_path):
        ...
        cols = {r[1] for r in conn.execute(
            "PRAGMA table_info(user_soul_events)"
        ).fetchall()}
...
class TestMemoryPhases:
    ...
    def test_reinforce_updates_last_used(self, tmp_path):
        ...
        conn.close()
        assert all(r[0] is not None for r in rows), "last_used_at should be set after reinforce"
...
class TestMoERouter:
    ...
    def test_backend_keywords_select_backend(self):
        from backend.moe_router_simple import MoERouter
        router = MoERouter()
        result = router.select_expert("I need help with FastAPI and SQL database migrations")
    ...
    def test_force_unknown_id_falls_back_to_auto(self):
        from backend.moe_router_simple import MoERouter
        router = MoERouter()
        result = router.select_expert("Help me with SQL", force_expert_id="nonexistent_expert")
    ...
    def test_matched_keywords_max_5(self):
        from backend.moe_router_simple import MoERouter
        router = MoERouter()
        result = router.select_expert("deploy ci/cd docker kubernetes monitoring aws cloud infrastructure")
        assert len(result["matched_keywords"]) <= 5

    def test_project_keywords_select_project_manager(self):
        from backend.moe_router_simple import MoERouter
        router = MoERouter()
        result = router.select_expert("I need a project roadmap and planning priorities for next sprint")
