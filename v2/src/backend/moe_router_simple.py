...
class MoERouter:
    ...
    def _create_experts(self) -> List[Expert]:
        ...
        return [
            ...
            Expert(
                ...
                prompt="Você é um assistente geral. Responda de forma clara e útil. Seja curto por padrão e só aprofunde quando o usuário pedir. Não faça apresentações longas nem repita o perfil do usuário, a menos que ele solicite.",
    ...
    def select_expert(
        ...
        for expert in self.experts:
            ...
            hits = [kw for kw in expert.keywords if kw.lower() in text_lower]
            matched[expert.id] = hits
        ...
        result = self._expert_to_dict(best_expert, best_score)
        kws = matched.get(best_expert_id, [])
        result["matched_keywords"] = kws[:5]
        result["selection_reason"] = (
            f"Matched: {', '.join(kws[:3])}" if kws else "Default — no strong keyword match"
        )
        return result
