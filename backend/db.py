...
class DatabaseManager:
    ...
    def _ensure_database(self):
        ...
        with self._connect() as conn:
            # Tabela de usuários (migration 001)
            conn.execute(
                """
            ...
            try:
                conn.execute(
                    ...
                    USING fts5(title, content_md, tags, note_id UNINDEXED)
                """
                )
                conn.execute(
                    """
                ...
                conn.execute(
                    """
                    CREATE TRIGGER IF NOT EXISTS notes_fts_au AFTER UPDATE OF title, content_md, tags ON notes
                    BEGIN
                      UPDATE notes_fts
                      SET title = new.title, content_md = new.content_md, tags = new.tags
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS projects (
                    ...
                    tap_json TEXT DEFAULT '{}',
                    expert_mode INTEGER DEFAULT 0,
                    stakeholders_json TEXT DEFAULT '[]',
                    risk_matrix_json TEXT DEFAULT '[]',
                    release_plan_json TEXT DEFAULT '[]',
                    team_charter_json TEXT DEFAULT '{}',
                    status_report_json TEXT DEFAULT '{}',
            ...
            # Executar migrações para banco de dados existente
            ...
            # Migração: colunas category e source na tabela notes
            try:
                conn.execute(
            ...
            # Migração: índices para notes (category, source)
            try:
                conn.execute(
            ...
            try:
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_notes_project_category ON notes(project_id, category)"
                )
            except Exception:
                pass
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_installed_mcps_user ON installed_mcps(user_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_installed_mcps_active ON installed_mcps(is_active)"
            )
            ...
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS project_wizard_state (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_wizard_state_conversation ON project_wizard_state(conversation_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_wizard_state_user ON project_wizard_state(user_id)"
            )
    ...
    def update_project_details(self, project_id: int, user_id: int, details: dict) -> None:
        if not details:
            return
        fields = []
        params = []
        for key, val in details.items():
            if key in {"name", "context_md", "tap_json", "expert_mode", "stakeholders_json", "risk_matrix_json", "release_plan_json", "team_charter_json", "status_report_json", "lessons_learned_json"}:
                fields.append(f"{key}=?")
                params.append(val)
        if not fields:
            return
        ...
        with self._connect() as conn:
            conn.execute(
                f"UPDATE projects SET {', '.join(fields)}, updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?",
                tuple(params)
            )
            conn.commit()
    ...
    # ── Project Wizard State Methods ───────────────────────────────────────────

    def save_wizard_state(self, conversation_id: int, user_id: int, state_json: str) -> None:
        """Salva ou atualiza o estado do wizard."""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO project_wizard_state 
                (conversation_id, user_id, state_json, updated_at)
                VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                """,
    ...
    def load_wizard_state(self, conversation_id: int) -> Optional[str]:
        """Carrega o estado do wizard como JSON string."""
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute(
                "SELECT state_json FROM project_wizard_state WHERE conversation_id=?",
                (conversation_id,),
            ).fetchone()
            return row[0] if row else None

    def delete_wizard_state(self, conversation_id: int) -> None:
        """Remove o estado do wizard."""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "DELETE FROM project_wizard_state WHERE conversation_id=?",
                (conversation_id,),
            )
            conn.commit()

    def cleanup_expired_wizard_states(self, hours: int = 24) -> int:
        """Remove estados expirados e retorna quantidade removida."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                """
                DELETE FROM project_wizard_state 
    ...
    def get_installed_mcps(self, user_id: int) -> List[Dict[str, Any]]:
        """Retorna todos os MCPs instalados para o usuário"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                """
                SELECT id, manifest_json, installed_at, is_active 
                FROM installed_mcps 
                WHERE user_id = ?
    ...
    def get_active_mcps(self, user_id: int) -> List[Dict[str, Any]]:
        """Retorna apenas MCPs ativos para o usuário"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                """
                SELECT id, manifest_json, installed_at 
                FROM installed_mcps 
    ...
    def install_mcp(self, user_id: int, mcp_id: str, manifest: Dict[str, Any]) -> bool:
        """Instala um MCP para o usuário"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    """
                    INSERT OR REPLACE INTO installed_mcps 
                    (id, user_id, manifest_json, is_active)
                    VALUES (?, ?, ?, 1)
                    """,
                    (mcp_id, user_id, json.dumps(manifest))
                )
                conn.commit()
                return True
        except Exception:
    ...
    def toggle_mcp(self, user_id: int, mcp_id: str, is_active: bool) -> bool:
        """Ativa/desativa um MCP instalado"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    """
                    ...
                    WHERE id = ? AND user_id = ?
                    """,
                    (1 if is_active else 0, mcp_id, user_id)
                )
                conn.commit()
    ...
    def uninstall_mcp(self, user_id: int, mcp_id: str) -> bool:
        """Remove um MCP instalado"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    """
                    DELETE FROM installed_mcps 
                    WHERE id = ? AND user_id = ?
                    """,
                    (mcp_id, user_id)
                )
    ...
    def get_mcp_manifest(self, user_id: int, mcp_id: str) -> Optional[Dict[str, Any]]:
        """Retorna o manifesto de um MCP específico"""
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute(
                """
    ...
    def update_mcp_manifest(self, user_id: int, mcp_id: str, manifest: Dict[str, Any]) -> bool:
        """Atualiza o manifesto de um MCP instalado"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    """
                    ...
                    WHERE id = ? AND user_id = ?
                    """,
                    (json.dumps(manifest, ensure_ascii=False), mcp_id, user_id)
                )
                conn.commit()
                return True
        except Exception:
    ...
    def create_note(
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                """
                INSERT INTO notes (user_id, project_id, title, content_md, tags, category, source)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (user_id, project_id, title, content_md, tags, category, source),
            )
            conn.commit()
    ...
    def list_notes(self, user_id: int, project_id: Optional[int] = None) -> List[Dict[str, Any]]:
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            if project_id is not None:
                rows = conn.execute(
                    "SELECT * FROM notes WHERE user_id = ? AND project_id = ? ORDER BY updated_at DESC",
                    (user_id, project_id),
                ).fetchall()
            else:
                rows = conn.execute(
    ...
    def search_notes(self, user_id: int, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
                """
...
def update_project_details(project_id: int, user_id: int, details: dict) -> None:
    db_manager.update_project_details(project_id, user_id, details)
...
def get_installed_mcps(user_id: int) -> List[Dict[str, Any]]:
    """Retorna todos os MCPs instalados para o usuário"""
    return db_manager.get_installed_mcps(user_id)


def get_active_mcps(user_id: int) -> List[Dict[str, Any]]:
    """Retorna apenas MCPs ativos para o usuário"""
    return db_manager.get_active_mcps(user_id)


def install_mcp(user_id: int, mcp_id: str, manifest: Dict[str, Any]) -> bool:
    """Instala um MCP para o usuário"""
    return db_manager.install_mcp(user_id, mcp_id, manifest)


def toggle_mcp(user_id: int, mcp_id: str, is_active: bool) -> bool:
    """Ativa/desativa um MCP instalado"""
    return db_manager.toggle_mcp(user_id, mcp_id, is_active)


def uninstall_mcp(user_id: int, mcp_id: str) -> bool:
    """Remove um MCP instalado"""
    return db_manager.uninstall_mcp(user_id, mcp_id)


def get_mcp_manifest(user_id: int, mcp_id: str) -> Optional[Dict[str, Any]]:
    """Retorna o manifesto de um MCP específico"""
    return db_manager.get_mcp_manifest(user_id, mcp_id)
...
# ── Project Wizard State Persistence ───────────────────────────────────────

def save_wizard_state(conversation_id: int, user_id: int, state: Dict[str, Any]) -> None:
    """Salva o estado do wizard no banco de dados."""
    state_json = json.dumps(state, ensure_ascii=False)
    db_manager.save_wizard_state(conversation_id, user_id, state_json)


def load_wizard_state(conversation_id: int) -> Optional[Dict[str, Any]]:
    """Carrega o estado do wizard do banco de dados."""
    state_json = db_manager.load_wizard_state(conversation_id)
    if state_json:
        return json.loads(state_json)
    return None


def delete_wizard_state(conversation_id: int) -> None:
    """Remove o estado do wizard do banco de dados."""
    db_manager.delete_wizard_state(conversation_id)


def cleanup_expired_wizard_states(hours: int = 24) -> int:
    """Remove estados de wizard expirados (mais antigos que N horas)."""
    return db_manager.cleanup_expired_wizard_states(hours)


def update_mcp_manifest(user_id: int, mcp_id: str, manifest: Dict[str, Any]) -> bool:
    """Atualiza o manifesto de um MCP instalado"""
    return db_manager.update_mcp_manifest(user_id, mcp_id, manifest)
...
def create_note(
    ...
    return db_manager.create_note(user_id, title, content_md, tags=tags, project_id=project_id,
...
def update_note(
    ...
    return db_manager.update_note(note_id, user_id, title, content_md, tags=tags, project_id=project_id,
                                  category=category, source=source)


def delete_note(note_id: int, user_id: int) -> bool:
    return db_manager.delete_note(note_id, user_id)


def get_note(note_id: int, user_id: int) -> Optional[Dict[str, Any]]:
    return db_manager.get_note(note_id, user_id)


def list_notes(user_id: int, project_id: Optional[int] = None) -> List[Dict[str, Any]]:
    return db_manager.list_notes(user_id, project_id=project_id)


def search_notes(user_id: int, query: str, limit: int = 10) -> List[Dict[str, Any]]:
    return db_manager.search_notes(user_id, query, limit=limit)
...
def update_project_status(
    ...
    return db_manager.update_project_status(project_id, user_id, status, suspension_reason)


def update_project_repository(project_id: int, user_id: int, repository_data: dict) -> bool:
    return db_manager.update_project_repository(project_id, user_id, repository_data)
...
def create_wiki_entry(
    ...
    return db_manager.create_wiki_entry(project_id, user_id, section, title, content_md, author)
...
def append_wiki_entry(
    ...
    return db_manager.append_wiki_entry(project_id, user_id, section, content_md, author, title)


def get_wiki_section(project_id: int, section: str, limit: int = 50) -> List[Dict[str, Any]]:
    return db_manager.get_wiki_section(project_id, section, limit=limit)


def get_wiki_full(project_id: int) -> Dict[str, List[Dict[str, Any]]]:
    return db_manager.get_wiki_full(project_id)


def initialize_project_wiki(project_id: int, user_id: int) -> None:
    return db_manager.initialize_project_wiki(project_id, user_id)
...
def log_agent_action(
    ...
    return db_manager.log_agent_action(project_id, user_id, agent_id, action, detail, source, external_tool)


def get_agent_log(project_id: int, limit: int = 100, agent_id: str = None) -> List[Dict[str, Any]]:
    return db_manager.get_agent_log(project_id, limit=limit, agent_id=agent_id)
...
def record_token_usage(
    ...
    return db_manager.record_token_usage(project_id, user_id, tokens_in, tokens_out,
        sprint_label=sprint_label, message_id=message_id,
        agent_id=agent_id, provider=provider, model=model)


def get_token_usage_summary(project_id: int) -> Dict[str, Any]:
    return db_manager.get_token_usage_summary(project_id)
