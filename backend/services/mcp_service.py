...
class MCPService:
    ...
    def validate_manifest(self, manifest: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "compatible_with" in manifest:
            ...
            if not isinstance(compatible_with, list):
                ...
                for item in compatible_with:
                    check = SecurityGuardrail.evaluate(str(item))
                    if check["action"] == "block":
                        errors.append(
                            f"Item em compatible_with contém conteúdo bloqueado pelo B.E.N. 2.0: {check['reason']}"
                        )
                        break
        else:
        ...
        # Sanitização de campos de texto via B.E.N. 2.0
        # Campos de texto livre que podem conter injection
        text_fields_to_check = ["name", "description", "id"]
        for field in text_fields_to_check:
            if field in manifest and isinstance(manifest[field], str):
                ...
                if check["action"] == "block":
                    errors.append(
                        f"Campo '{field}' contém conteúdo bloqueado pelo B.E.N. 2.0: {check['reason']}"
                    )

        # Sanitização dos itens da lista 'tools'
        if "tools" in manifest and isinstance(manifest["tools"], list):
            for i, tool in enumerate(manifest["tools"]):
                check = SecurityGuardrail.evaluate(str(tool))
                if check["action"] == "block":
                    errors.append(
                        f"Tool[{i}] contém conteúdo bloqueado pelo B.E.N. 2.0: {check['reason']}"
                    )
