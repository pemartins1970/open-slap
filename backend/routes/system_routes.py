...
async def get_system_map(
    ...
    ascii_map = _get_main()._generate_system_map_ascii()
...
async def doctor_check(
    ...
    profile_markdown = profile.get("markdown", "")

    # Verificar configurações LLM
    from ..deps import _infer_env_provider
    ...
    async with aiohttp.ClientSession() as session:
        # Testar provedor ativo: primeiro user settings, depois inferir do .env
        provider = safe_settings.get("provider") or _infer_env_provider()
        api_key = _get_user_api_key(user_id, provider) if provider else None
        if not api_key and provider:
            api_key = _get_env_api_key_for_provider(provider)
        # Se ainda sem chave, tentar cada provider do env
        if not api_key:
            for p in ["gemini", "groq", "openai", "openrouter"]:
                api_key = _get_env_api_key_for_provider(p)
                if api_key:
                    provider = p
                    break
        if api_key:
    ...
    checks_list = [
        {
            ...
            "ok": bool(profile),
            "detail": "Coletado" if profile else "Sem perfil de sistema",
            "status": "ok" if profile else "warn",
        },
        {
            "id": "llm_configured",
            "label": "LLM configurado",
            "ok": bool(api_key),
            "detail": "Chave encontrada" if api_key else "Nenhuma chave configurada",
            "status": "ok" if api_key else "warn",
        },
        {
            "id": "api_reachable",
            "label": "API do provedor",
            "ok": any(r["ok"] for r in connectivity_results),
            "detail": "Conectado" if any(r["ok"] for r in connectivity_results) else "Sem conectividade",
            "status": "ok" if any(r["ok"] for r in connectivity_results) else "error",
        },
    ]
    report = {
        "ok": all(c["ok"] for c in checks_list),
        "timestamp": datetime.utcnow().isoformat(),
        "user_id": user_id,
        "system_profile_summary": summary,
        "system_profile_markdown": profile_markdown,
        "llm_settings": {
