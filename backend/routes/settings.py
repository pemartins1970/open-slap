...
@router.post("/api/settings/llm/keys/test")
async def test_llm_key_endpoint(
    payload: LlmProviderKeyCreateInput,
    credentials: HTTPAuthorizationCredentials = Depends(security),
):
    """Testa se a API Key informada para um provedor é válida fazendo uma chamada leve."""
    token = credentials.credentials
    current_user = get_current_user(token)
    if not current_user:
        raise HTTPException(status_code=401, detail="Não autenticado")
    ...
    if not key:
        raise HTTPException(status_code=400, detail="Chave API vazia")
    ...
    async with aiohttp.ClientSession() as session:
        try:
            if prov == "gemini":
                ...
                async with session.post(url, json={"contents": [{"parts": [{"text": "ping"}]}]}) as r:
                    if r.status == 403 or r.status == 401:
                        return {"ok": False, "error": "Chave Gemini inválida ou sem permissão"}
                    if r.status != 200:
                        err_text = await r.text()
                        return {"ok": False, "error": f"Gemini retornou status {r.status}: {err_text[:100]}"}
                    return {"ok": True}
            elif prov == "openai":
                ...
                async with session.get(url, headers=headers) as r:
                    if r.status == 401:
                        return {"ok": False, "error": "Chave OpenAI inválida"}
                    if r.status != 200:
                        err_text = await r.text()
                        return {"ok": False, "error": f"OpenAI retornou status {r.status}: {err_text[:100]}"}
                    return {"ok": True}
            elif prov == "groq":
                ...
                async with session.get(url, headers=headers) as r:
                    if r.status == 401:
                        return {"ok": False, "error": "Chave Groq inválida"}
                    if r.status != 200:
                        err_text = await r.text()
                        return {"ok": False, "error": f"Groq retornou status {r.status}: {err_text[:100]}"}
                    return {"ok": True}
            elif prov == "anthropic":
                ...
                body = {"model": "claude-3-haiku-20240307", "messages": [{"role": "user", "content": "ping"}], "max_tokens": 5}
                async with session.post(url, headers=headers, json=body) as r:
                    if r.status == 401:
                        return {"ok": False, "error": "Chave Anthropic inválida"}
                    if r.status != 200:
                        err_text = await r.text()
                        return {"ok": False, "error": f"Anthropic retornou status {r.status}: {err_text[:100]}"}
                    return {"ok": True}
                ...
                async with session.get(url, headers=headers) as r:
                    if r.status == 401:
                        return {"ok": False, "error": "Chave OpenRouter inválida ou sem créditos"}
                    if r.status != 200:
                        err_text = await r.text()
                        return {"ok": False, "error": f"OpenRouter retornou status {r.status}: {err_text[:100]}"}
                    return {"ok": True}
            else:
                return {"ok": False, "error": f"Provedor {prov} não suporta teste rápido"}
        except Exception as e:
            return {"ok": False, "error": f"Erro de rede: {str(e)}"}


@router.get("/api/settings/plan")
async def get_plan_settings_endpoint(
    credentials: HTTPAuthorizationCredentials = Depends(security),
):
    token = credentials.credentials
    current_user = get_current_user(token)
    if not current_user:
        raise HTTPException(status_code=401, detail="Não autenticado")
    return {
        "auto_approve_informational": app_settings.plan_auto_approve_informational,
        "auto_approve_all": app_settings.plan_auto_approve_all,
    }
