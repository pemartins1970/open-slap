...
class Settings:
    ...
    def __init__(self):
        ...
        # Ideation
        self.IDEATION_CAPTURE_ENABLED = _env_flag(
            os.getenv("OPENSLAP_IDEATION_CAPTURE_ENABLED"), True
        )

        # AI Gateway
        self.AI_GATEWAY_ENABLED = _env_flag(
            os.getenv("OPENSLAP_AI_GATEWAY_ENABLED"), False
        )
        self.AI_GATEWAY_URL = str(
            os.getenv("OPENSLAP_AI_GATEWAY_URL") or "http://localhost:8000"
        )
        self.AI_GATEWAY_API_KEY = str(
            os.getenv("OPENSLAP_AI_GATEWAY_API_KEY") or ""
        )

        # Plan approval settings
        self.plan_auto_approve_informational = _env_flag(
            os.getenv("OPENSLAP_PLAN_AUTO_APPROVE_INFORMATIONAL"), True
        )
        self.plan_auto_approve_all = _env_flag(
            os.getenv("OPENSLAP_PLAN_AUTO_APPROVE_ALL"), False
        )
