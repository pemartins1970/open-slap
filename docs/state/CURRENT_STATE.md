...
### Sessão Atual (2026-06-08)

| Item | Arquivos / Localização | Status |
|------|----------------------|--------|
| P-01 — `plan_auto` detection | `App_auth.jsx` — `parseMessageBlocks` detecta ````plan_auto`, emite `type: plan_auto` | ✅ |
| Cleanup — `planAutoApproveInformational` | `SystemSettingsPanel.jsx`, `App_auth.jsx` — toggle removido (inoperante) | ✅ |
| Cleanup — `buildStartProjectPrompt` | `App_auth.jsx`, `OnboardingModal.jsx` — função órfã removida | ✅ |
...
- **2026-06-02** — Fix B-01 (Opção B): contexto real na orquestração, WebSocket em tempo real, session_id no frontend. CHANGELOG + CURRENT_STATE + DEV_JOURNAL atualizados.
